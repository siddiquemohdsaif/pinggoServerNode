...
router.post('/', async (req, res) => {
    try {
        ...
        const updatedDocUser = await UserLock.getInstance().run(uid, async () => {
            return await userDataModifier.updateUserName(uid, userName);
        });

        if (updatedDocUser) {
            return res.status(200).json(updatedDocUser);
        } else {
            return res.status(400).json({ success: false, message: "Unable to update userName." });
        }
    } catch (error) {
        console.error("Error in change userName:", error.message);
