...
const ULARecorderHandler = require("../utils/ULARecorderHandler");
const AES = require("../utils/AES_256");
const newULARecorderHandler = new ULARecorderHandler();
const router = express.Router();
const UserLock = require('../utils/Lock/UserLock');

router.post('/uploadPacketToULARSession', async (req, res) => {
    try {
        ...
        const updatedDocUser = await UserLock.getInstance().run(uid, async () => {
            return await newULARecorderHandler.ULARecord(uid, sessionKey,packet);
        });
